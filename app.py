            ...
            if st.button("💾 Forcer sauvegarde (compressée)", key="force_save_debug"):
                with st.spinner("Sauvegarde..."):
                    try:
                        ...
                        # Vérifier la limite Google Sheets (50000 caractères)
                        if len(envelope) > 50000:
                            st.error(f"⚠️ Données trop grosses ({len(envelope):,} > 50,000 limite Google)")
                        else:
                            st.success(f"✅ Taille OK pour Google Sheets")
